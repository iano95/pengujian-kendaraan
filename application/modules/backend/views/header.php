...
<head>
  ...
  <script src="<?=base_url()?>_temp/backend/js/printThis.js"></script>
      ...
      var t_type    = '<?=$this->session->flashdata('t_type');?>';
        ...
        if (t_message.length > 0) {
          ...
          swal(t_message, {
            icon:t_type
            <?php 
            $this->session->set_flashdata('t_message',''); 
            $this->session->set_flashdata('t_type', '');
            ?>
  ...
  <div class="container-scroller">
    <nav class="navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
      <div class="text-center navbar-brand-wrapper d-flex align-items-center justify-content-center">
        <a class="navbar-brand brand-logo" href="<?=site_url("backend/dashboard")?>">Pengujian Kendaraan</a>
        <a class="navbar-brand brand-logo-mini" href="<?=site_url("backend/dashboard")?>">P</a>
      </div>
